...
// Variable global para guardar los datos en memoria y no leer el archivo en cada consulta
...
async function getClientDataFromDrive(drive) {
    try {
        ...
        // Suponiendo que el contenido es texto (JSON)
...
app.get('/api/expediente/:dni', async (req, res) => {
    ...
    try {
        ...
        // Buscar el cliente por DNI en los datos cacheados
        const cliente = cachedClientData.find(c => String(c.dni).trim() === String(dniBuscado).trim());

        if (cliente) {
            // Si se encuentra, devolver solo la información de ese cliente
            res.json(cliente);
        } else {
